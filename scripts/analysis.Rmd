...
Import data
```{r}
setwd("/home/adam/cloud/gdrive/fire_project/DS421_proj")
df <- read_csv('data/FireData87and88_27_fires.csv', col_types = cols(clusterID = col_factor()))
```
...
df_nbr <- df %>%
  ...
  gather(years, key = "year", value = "NBR") %>%
...
plot NBR through for a random sample of patches
```{r}
...
Calculating the percent NBR recovery for each pixel
...
Histogram of percent recovery in 2017 (30 years after fire)
```{r}
df %>%
...
Looking at the percent NBR recovery in 2017 across burn severity classes
```{r}
df <- df %>%
